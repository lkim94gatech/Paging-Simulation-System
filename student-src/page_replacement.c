...
/*  ----------------------------------------------------------------------------------
    ...
    Call the page replacement algorithm to identify an
...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        ...
        pte_t* entry = (pte_t*)((ptbr * PAGE_SIZE) + mem);
        entry += frame_table[victim_pfn].vpn;                       
        if (entry->dirty) {                                         
            swap_write(entry, (void*) (victim_pfn * PAGE_SIZE + mem));
            stats.writebacks++;
        }
        entry->valid = 0;                                         
